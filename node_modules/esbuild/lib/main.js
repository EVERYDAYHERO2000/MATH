...
function flagsForBuildOptions(options, isTTY2, logLevelDefault, writeDefault) {
  var _a;
  let flags = [];
...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.21") {
        throw new Error(`Cannot start service: Host version "${"0.8.21"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
  ...
  return {
    ...
    afterClose,
    service: {
      buildOrServe(serveOptions, options, isTTY2, callback) {
        const logLevelDefault = "info";
        try {
          let key = nextBuildKey++;
          let writeDefault = !streamIn.isBrowser;
          let [flags, write, plugins, stdin, resolveDir, incremental] = flagsForBuildOptions(options, isTTY2, logLevelDefault, writeDefault);
          let request = {command: "build", key, flags, write, stdin, resolveDir, incremental};
...
var startService = (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.21"}`), {
    cwd: process.cwd(),
    windowsHide: true,
  ...
  return Promise.resolve({
    build: (options2) => new Promise((resolve, reject) => service.buildOrServe(null, options2, isTTY(), (err, res) => err ? reject(err) : resolve(res))),
    serve: (serveOptions, buildOptions) => {
      if (serveOptions === null || typeof serveOptions !== "object")
        throw new Error("The first argument must be an object");
      return new Promise((resolve, reject) => service.buildOrServe(serveOptions, buildOptions, isTTY(), (err, res) => err ? reject(err) : resolve(res)));
    },
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.21"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
